...
function Elogio() {
  ...
  const handleSubmit = (e) => {
    ...
    api.post('/elogios', payload)
      .then(() => navigate('/confirmacao'))
      .catch(async (err) => {
        try {
          if (err && err.status === 401) {
            alert('Você precisa estar autenticado para enviar o elogio. Faça login e tente novamente.');
            return;
          }
          const body = await err.json();
          alert(body?.message || 'Erro ao enviar o elogio. Tente novamente.');
        } catch {
          alert('Erro ao enviar o elogio. Tente novamente.');
        }
      });
  };
  ...
  const handleAnonimoSubmit = (e) => {
    e.preventDefault();
    alert('Envio anônimo não está disponível. Faça login para enviar seu elogio.');
  };
